function [RealE, HeapPos] = VMT_CalHeapPos_3(NormE, CompStatus)
...
%   CompStatus      补偿情况，1表示有补偿，0表示无补偿
    ...
    [F_snap_Mat, U_snap_Mat] = VMT_SingleGetFm(1, HMat, 3);
    ...
    for i = 1: UnitSum
        ...
        for j = 1: UnitSum
            ...
            IsDown = (j < i);
            if (j == i + 1)
                U_all(j) = VMT_SingleGetU(RealE(j), HMat(IfThatComp + 2), RealE(i) * F_snap_Mat(IfThisComp + 2), 0, -3);
            else
                U_all(j) = VMT_SingleGetU(RealE(j), HMat(IfThatComp + 2), RealE(i) * F_snap_Mat(IfThisComp + 2), IsDown, 3);
            end
        end
        HeapPos(i) = sum(U_all);
    end
